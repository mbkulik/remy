...
#include "network.cc"
#include "handbuiltcc-templates.cc"
...
ConfigRange configuration_range;
  ...
  for(int i = 0; i < num_runs; i++) {
    ...
    //double tmp_score = 0.0;
      for ( auto &x : configs ) {
        /* run once */
        Network<HandBuiltCC, HandBuiltCC> network1( HandBuiltCC(), run_prng, x);

        network1.run_simulation( ticks_to_run );

        score += network1.senders().utility();
        throughputs_delays.emplace_back( x, network1.senders().throughputs_delays() );
      }
  }
